...
export default function SeasonScheduleRow({
  season,
  seasonLabel,
  value,
  error,
  onChange,
}: SeasonScheduleRowProps) {
  const handleNumberChange = (key: "watering_interval" | "fertilizing_interval") => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="grid gap-3 rounded-xl border border-neutral-200 bg-neutral-50 p-4 md:grid-cols-[140px_1fr_1fr] md:items-center">
      ...
      <div>
        ...
        <Input
          ...
          aria-describedby={
            error?.fertilizing_interval ? `fertilizing-${season}-error` : `fertilizing-${season}-help`
          }
        />
